...
### ③ Continuous distillation (distillation isn't one-off)

When there are new conversations: append the new export to `input/` → log candidates per `schemas/inbox-v2.json` → `python3 distill_audit.py audit` to build the evidence bundle and coverage report → read [`prompts/rediscovery.md`](prompts/rediscovery.md) to find new patterns / conflicts / time-based changes → `python3 distill_audit.py verify reports/latest` to validate sources and citations → update `canonical/` after item-by-item confirmation → re-run `build.py` (and `install.py` write-back if needed).
