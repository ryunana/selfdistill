...
有新对话后：把新导出追加到 `input/` → 按 `schemas/inbox-v2.json` 记候选 → `python3 distill_audit.py audit` 生成证据包与覆盖报告 → 读 [`prompts/rediscovery.md`](prompts/rediscovery.md) 发现新规律 / 冲突 / 时间变化 → `python3 distill_audit.py verify reports/latest` 校验来源与引用 → 逐条确认后更新 `canonical/` → 重跑 `build.py`（需要时再 `install.py` 写回）。
