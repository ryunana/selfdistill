...
> **已有自动导入器**：运行 `python3 import_chats.py --source chatgpt|gemini|deepseek|local --path <导出>` 可自动把 ChatGPT / Gemini / DeepSeek 导出与本地 Codex / Claude Code 会话整理成下面的统一格式写入 `input/`（本地来源先 dry-run 清单、确认后写入）。Gemini 按每次 `Prompted` 活动拆分；ChatGPT 和 DeepSeek 的导出分支会拆成独立会话。活动边界或本地 JSONL 来源无法可靠识别时，导入器会停止该输入而非猜测；本地混合目录可用 `--local-format auto|codex|claude` 指定。本文档的整理说明保留为手工 fallback。
...
自动导入时仅保留活动分支；图片变成 `[图片]`，未知结构化内容变成 `[未识别附件]`，不会把资产地址或原始字典写进 Markdown。
...
这些来源的记录保存在本机目录，可以把整理工作交给正在使用的 AI 工具协助：把记录交给 AI，让它按上面的统一格式整理成 Markdown（原始记录仍只放在 `input/`）。自动导入只排除可确认的系统环境、命令、元数据和任务通知；普通用户写的 XML、代码或提及这些标记的文字会保留。
