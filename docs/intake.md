...
自动导入时，`current_node` 有效就仅保留该活动分支；只有它缺失或无效时，才保留并拆分已验证的根到叶分支。图片变成 `[图片]`，未知结构化内容变成 `[未识别附件]`，不会把资产地址或原始字典写进 Markdown。
