# selfstill × DeepSeek Harness 支持实施计划
...
- `dsh/package.json` —— selfstill-dsh，`dsh.bundle.patch`，零依赖
- `dsh/cordis.patch.yml` —— insert 自身
- `dsh/index.mjs` —— Cordis entry，`inject: ['skills']`，运行时注册 `selfstill` skill
- `dsh/skills/selfstill/SKILL.md` —— <500 行工作流指南
- `dsh/skills/selfstill/references/workflow.md` —— 详细分步
...
- `docs/superpowers/specs/2026-08-17-dsh-support-design.md` —— L4 skill 名改为 `selfstill-<领域>` 的一致性修正
...
└── skills/
    ├── selfstill-decision-logic/SKILL.md
    ├── selfstill-user-profile/SKILL.md
    └── selfstill-<fid>/SKILL.md      # 每个 L4 打法
```
...
- L4 复用 canonical frontmatter 的 `description`（已是 "Use when..." 路由风格），skill 名 `selfstill-<fid>`
...
- 默认 opener：`You are a coding agent powered by the {{model}} model. Your working directory is {{cwd}}.`，环境变量 `SELFSTILL_DSH_PERSONA_OPENER` 覆盖
...
- `package.json`：`name: selfstill-dsh`，`type: module`，`main: ./index.mjs`，`exports`，`files: [index.mjs, cordis.patch.yml, skills]`，`dsh.bundle.patch: ./cordis.patch.yml`，零依赖，MIT
- `cordis.patch.yml`：`- insert: - id: selfstill-dsh, name: 'selfstill-dsh'`
- `index.mjs`：`export const name/inject/apply`；`ctx.effect(() => ctx.skills.register({ name: 'selfstill', description, whenToUse, content, resourceBase }))`；正文从包内 `skills/selfstill/SKILL.md` 读取（`node:fs` + `import.meta.url`）
...
- 「DeepSeek Harness 支持」章节：写回用法（`--target dsh` + 隐私说明）、插件一行安装命令（git 源 `github:ryunana/selfstill#main&path:/dsh`，注明 npm 发布后可用 `dsh plugin --profile web add selfstill-dsh`）
...
- `gh repo edit ryunana/selfstill --add-topic dsh-plugin`
- 询问用户是否 push 到 GitHub（本地已完成全部验证后再 push）

## 4. 验收标准

- `--target dsh` 安装后：`$DSH_HOME/cordis.patch.yml` 的 `system-prompt.persona` 只含 L1 + 开场白；`$DSH_HOME/skills/` 出现 selfstill-* skill；重复安装幂等；无标记文件被保护
- `dsh plugin --profile web add "github:ryunana/selfstill#<ref>&path:/dsh"` 安装后，DSH agent 目录中出现 `selfstill` skill（重启 web 生效）
- README 提供可直接复制的安装命令
- 现有测试与发布扫描通过
